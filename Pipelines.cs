...
namespace PipelineTemplate
{
    public partial class Pipelines
    {
        ...
        public ActionBlock<OutputPipelineParams> EndPipeline
        {

            get => new ActionBlock<OutputPipelineParams>(Command =>
            {
                ...
                Command.Responder.CloseResponse("");
            });
        }
        public TransformBlock<InitServerFlow, BuildFlow> ConfigServer
        {
            get
            {
                return new TransformBlock<InitServerFlow, BuildFlow>(Config =>
                {
        ...
        public TransformBlock<BuildFlow, OutputFlow> BuildConnectSet
        {
            get
            {
                return new TransformBlock<BuildFlow, OutputFlow>(async Server =>
                {
        ...
        public TransformBlock<OutputFlow, OutputPipelineParams> GetOutputParams
        {
            get
            {
                return new TransformBlock<OutputFlow, OutputPipelineParams>(async ConnectionSet =>
                {
                    while (!ConnectionSet.Token.IsCancellationRequested)
                    {
                        ...
                        if (t is Task<HttpListenerContext>)
                        {
                            ...
                            EndPipeline.Post(new OutputPipelineParams(new HttpContextResponder(context), Encoding.UTF8.GetBytes(context.Request.RawUrl), ConnectionSet.Token));
                        }
                    }
                    return default;
                });
